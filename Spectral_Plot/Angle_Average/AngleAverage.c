...
int main(int argc, char* argv[]) {
  ...
  for (k=0; k<180; k++) {
    ...
    for (i=0; i<NumLam; i++) {
       ...
       // Use forward finite-difference for dlambda
       if (i==0) {
      ...
      for (j=2; j<Nlayer-3; j++) {
       ...
       // Fresnel p-pol reflection coefficient (which is complex if there are absorbing layers)
       rP = m21P/m11P; 
       // Fresnel s-pol reflection coefficient
       rS = m21S/m11S;

       // Fresnel p-pol transmission coefficient (also complex if there are absorbing layers)
       tP = 1./m11P;
       // Fresnel s-pol transmission coefficient
       tS = 1./m11S;

       // P-pol Reflectance, which is a real quantity between 0 and 1
       RP = creal(rP*conj(rP));
       // S-pol Reflectance
       RS = creal(rS*conj(rS));
       // P-pol Reflectance
       TP = creal(tP*conj(tP))*n2*creal(cosLP)/(n1*cos(thetaI));
       // S-pol Reflectance
       TS = creal(tS*conj(tS))*n2*creal(cosLS)/(n1*cos(thetaI));
       // P-pol Absorbance
       AP = 1 - RP - TP;
       // S-pol absorbance
       AS = 1 - RS - TS;
       ...
       // Numerator of Spectral Efficiency and the Useful Spectral Density term
       if (lambda<=lbg) {       
     ...
     /*
     // P-Polarized spectral efficiency and PU
     SEP = SpectralEfficiency(EmissP, NumLam, LamList, lbg, Temp, &PUP);
     // S-Polarized spectral efficiency and PU
     SES = SpectralEfficiency(EmissS, NumLam, LamList, lbg, Temp, &PUS);
     printf("  SEP is %12.10f\n",SEP);
     printf("  SES is %12.10f\n",SES);
     PUsum += pi*(cos(thetaI)*sin(thetaI)*PUP*dtheta + cos(thetaI)*sin(thetaI)*PUS);
     SEsum += pi*(cos(thetaI)*sin(thetaI)*SEP*dtheta + cos(thetaI)*sin(thetaI)*SES);
     */
   } 

   printf("  SE:  %12.10f\n",SE_Num_sum/SE_Den_sum);
   printf("  PU:  %12.10f\n",SE_Num_sum);   
