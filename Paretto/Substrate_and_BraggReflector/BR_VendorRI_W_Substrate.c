...
int main(int argc, char* argv[]) {
  ...
  SEA = (double*)malloc((numFac*numNlayers*numFac*sizeof(double)));
  SDA = (double*)malloc((numFac*numNlayers*numFac*sizeof(double)));
  PF  = (int*)malloc((numFac*numNlayers*numFac*sizeof(int)));
    ...
    for (F1=0; F1<numFac; F1++) {
      ...
      for (F2=0; F2<numFac; F2++) {
        ...
        for (NI=0; NI<numNlayers; NI++) { 
          ...
          // Spacer Layer
          d[Nlayer-3] = 0.01;
          // Temporary RI for spacer layer - will replace with alumina
          rind[Nlayer-3] = 1.0 + 0.*I;
         ...
         printf("  %8.6f    %8.6f    %8.6f    %8.6f %f    %i     %12.10f  %12.10e\n",
                 fac1, fac2, d1*fac1, d2*fac2, Temp, Nlayer, SE,     PU);
         SEA[F1*numFac*numFac+F2*numFac+NI] = SE;
         SDA[F1*numFac*numFac+F2*numFac+NI] = PU;
  ...
  for (NI=0; NI<numNlayers; NI++) {
    ...
    i = TK*numFac*numFac+F1*numFac*numFac+F2*numFac+NI;
    // id is 1 if member i is dominated by at least one other member j!=i
    // a member is pareto optimal only if it is NOT dominated 
    id = IsDominated(i, numFac*numNlayers*numFac, SEA, SDA);
    if (id) PF[i] = 0;
    else {
      PF[i] = 1;
      fprintf(pf," %8.6f  %8.6f  %f     %f       %f   %i     %12.10f  %12.10e\n",
                 d1*SFAC[F1],d2*SFAC[F2] ,SFAC[F1],SFAC[F2],Temp,NLa[NI],SEA[i],SDA[i]);
    }
